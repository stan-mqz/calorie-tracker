...
export default function ({ activities, dispatch }: ActivityListProps) {
  const cathegoryName = useMemo(
    () => (cathegory: Activity["cathegory"]) =>
      cathegories.map((cat) => (cat.id === cathegory ? cat.name : "")),
    [activities]
  );
          ...
          <div className="space-y-2 relative">
            <p
              className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold ${
                activity.cathegory === 1 ? "bg-lime-500" : "bg-orange-500"
              }`}
            >
